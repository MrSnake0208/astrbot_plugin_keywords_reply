...
* `whitelist`: 允许管理插件的用户 QQ 号列表（非管理员也可用指令）。
* `words_limit`: 每个关键词允许添加的最大回复词条数（默认 10）。
* `case_sensitive`: 检测词匹配时是否区分大小写。
* `quote_reply`: 回复时是否引用触发消息（默认关闭）。
* `recall_delay`: 自动撤回延迟（秒）。格式：`"关键词延迟 检测词延迟"`。默认 `"0 0"` 表示不撤回。注意：超过 120 秒需 Bot 具有管理员权限（仅支持 OneBot 平台）。
